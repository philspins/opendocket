...
// NBFormerMembersURL is the canonical URL for former NB MLAs.
const NBFormerMembersURL = "https://www.legnb.ca/en/members/former"

// CrawlNewBrunswickFormerMembersFromWebsite scrapes former MLA profiles from the
// New Brunswick Legislative Assembly website. Former members appear in votes from
// previous sessions but are not present in the Represent API.
func CrawlNewBrunswickFormerMembersFromWebsite(indexURL string, client *http.Client) ([]MemberProfile, error) {
	if indexURL == "" {
		indexURL = NBFormerMembersURL
	}
	if client == nil {
		client = utils.NewHTTPClient()
	}
	doc, err := fetchDoc(indexURL, client)
	if err != nil {
		return nil, fmt.Errorf("nb former members website: %w", err)
	}
	...
	clog.Infof("[members] fetched %d former NB members from website", len(profiles))
	return profiles, nil
}
